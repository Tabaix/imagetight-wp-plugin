...
class ITC_Pro_Companion {
    ...
    public function admin_page_html() {
        ...
        <style>
            ...
            .metric-card { background: #fff; border-radius: 12px; padding: 25px; border: 1px solid #e2e8f0; display: flex; flex-direction: column; align-items: center; justify-content: center; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); }
            ...
            <div id="tab-dashboard" class="itc-tab-content active">
                ...
                <div class="dashboard-metrics">
                    ...
                    <div class="metric-card">
                        ...
                        <div class="metric-val" style="color: #ef4444;"><?php echo intval($pending_count); ?></div>
                    </div>
                </div>
                    ...
                    <?php
                        ...
                        if ($optimized_attachments->have_posts()) {
                            foreach($optimized_attachments->posts as $att) {
                                ...
                                $has_backup = (!empty($backup_path) && file_exists($backup_path));
                    ...
                    <div style="display:grid; gap: 20px; max-width: 600px;">
                        ...
                        <div>
                            ...
                            <select id="itc-format" style="width:100%; padding: 12px; font-size:14px; border-radius: 6px; border: 1px solid #cbd5e1;">
                                ...
                                <option value="heic" <?php selected($f, 'heic'); ?>>HEIC (Apple Format)</option>
                            </select>
                        </div>
                ...
                if ( file_exists( $file ) ) { 
                    ...
                    if ( $size > $threshold ) {
                        ...
                        $heavy_images[] = array(
                            ...
                            'size_raw'  => intval($size),
                            'size'      => size_format( $size, 2 ),
                            'thumb'     => esc_url(wp_get_attachment_image_url( $post->ID, 'medium' )),
    ...
    public function ajax_restore_image() {
        ...
        if ( copy( $backup_path, $original_filepath ) ) {
            ...
            wp_send_json_success( array( 'message' => 'Original restored.' ) );
        }
    ...
    private function remote_compress($file_path) {
        ...
        $payload .= "--" . $boundary . "\r\n";
        $payload .= "Content-Disposition: form-data; name=\"domain\"\r\n\r\n" . site_url() . "\r\n";
        $payload .= "--" . $boundary . "\r\n";
        ...
        if ( file_exists($old_file) && in_array($mime_type, ['image/jpeg', 'image/png']) ) {
            ...
            if (!is_wp_error($response) && wp_remote_retrieve_response_code($response) == 200) {
                ...
                if (get_option('itc_backup_originals', 1)) {
                    ...
                    update_post_meta($attachment_id, '_itc_backup_path', $old_file . '.itc-bak');
                } else {
                    wp_delete_file( $old_file );
                }
                
                if ( isset($metadata['sizes']) ) {
                    foreach ( $metadata['sizes'] as $size => $size_info ) wp_delete_file( $path_info['dirname'] . '/' . $size_info['file'] );
                }
                ...
                self::replace_urls_in_db($attachment_id, basename($new_filepath), $old_file);
                return $new_metadata; 
            }
        }
        return $metadata; 
    ...
    public function ajax_process_image() {
        ...
        if (!is_wp_error($response) && wp_remote_retrieve_response_code($response) == 200) {
            ...
            wp_send_json_error( "API Error: " . wp_remote_retrieve_response_message($response) );
        }
    }
}
