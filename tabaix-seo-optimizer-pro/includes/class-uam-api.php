...
class UAM_API
{
    ...
    public static function gemini_generate_with_image($prompt, $image_path, $model = null)
    {
        ...
        $b64 = base64_encode(file_get_contents($image_path));
        return self::gemini_generate($prompt, $model, [
            'image_base64' => $b64,
            'image_mime' => $mime,
        ]);
     ...
     *
     * Primary: gemini-2.5-flash-image via generateContent with responseModalities: ["Image"]
     * Fallback: imagen-4.0-generate-001 via :predict endpoint
     *
     * @param string $prompt       The image prompt.
     * @param string $aspect_ratio Aspect ratio (e.g., '1:1', '16:9', '4:3').
     * @param int    $count        Number of images (1-4).
     * @return array|WP_Error      Array of data URIs or WP_Error.
     */
    public static function gemini_generate_image($prompt, $aspect_ratio = '1:1', $count = 1)
    {
        $api_key = UAM_Settings::get('gemini_api_key');
        if (empty($api_key)) {
     ...
     * Uses the standard generateContent endpoint with responseModalities set to Image.
     */
     ...
     * Fallback if Gemini Flash Image is unavailable.
     */
    ...
    public static function openai_generate_with_image($prompt, $image_path_or_url, $model = null)
    {
        ...
        if (filter_var($image_path_or_url, FILTER_VALIDATE_URL)) {
            ...
            $b64 = base64_encode(file_get_contents($image_path_or_url));
    ...
    public static function generate($prompt, $system = '', $options = [])
    {
        ...
        if ($provider === 'openai') {
            ...
            if (is_wp_error($result) && $result->get_error_code() === 'no_key') {
                $gemini_key = UAM_Settings::get('gemini_api_key');
                if (!empty($gemini_key)) {
                    $full = $system ? "{$system}\n\n{$prompt}" : $prompt;
        ...
        if (is_wp_error($result) && $result->get_error_code() === 'no_key') {
            $openai_key = UAM_Settings::get('openai_api_key');
            if (!empty($openai_key)) {
    ...
    /**
     * Generate text with an image — routes to correct multimodal API.
     */
    ...
    {
        $provider = $provider ?? UAM_Settings::get('provider', 'gemini');
    ...
    /**
     * Returns the list of supported Gemini models (name => label).
     */
    public static function gemini_models()
    {
        return [
            ...
            // Fast & Cost-Effective (Recommended for daily use)
    ...
    /**
     * Returns the list of supported OpenAI models (name => label).
     */
