...
class UAM_Alt_Text
{
    ...
    const META_KEY = '_uam_ai_alt_generated';
    ...
    public function render_media_column($column_name, $attachment_id)
    {
        ...
        echo '<br><a href="#" class="uam-gen-alt-link" data-id="' . esc_attr($attachment_id) . '" style="font-size:11px;color:#6366f1;text-decoration:none">✦ Generate</a>';
        echo '</div>';
    }
    ...
    public function maybe_auto_generate($attachment_id)
    {
        ...
        if (!UAM_Settings::get('alt_text_auto', 0))
            return;
        if (!wp_attachment_is_image($attachment_id))
            return;
