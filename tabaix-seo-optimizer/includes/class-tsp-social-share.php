...
class TSP_Social_Share
{
    ...
    {
        register_setting('tabaix-seo-optimizer', 'tsp_ss_enabled',   ['sanitize_callback' => 'absint',             'default' => 1]);
        register_setting('tabaix-seo-optimizer', 'tsp_ss_position',  ['sanitize_callback' => 'sanitize_text_field','default' => 'below_content']);
        register_setting('tabaix-seo-optimizer', 'tsp_ss_networks',  ['sanitize_callback' => [$this, 'sanitize_networks'], 'default' => ['facebook','twitter','whatsapp','linkedin','pinterest','telegram','reddit','email','copy_link']]);
        register_setting('tabaix-seo-optimizer', 'tsp_ss_label',     ['sanitize_callback' => 'sanitize_text_field','default' => 'Share this post:']);
        register_setting('tabaix-seo-optimizer', 'tsp_ss_show_count',['sanitize_callback' => 'absint',             'default' => 0]);
    }
    ...
    public function enqueue_assets()
    {
        ...
        if (!get_option('tsp_ss_enabled', 1)) return;

        wp_enqueue_style(
            'tsp-social-share',
            plugins_url('../assets/css/tsp-social-share.css', __FILE__),
            [],
            '1.1.0'
        );
        ...
        wp_enqueue_script(
            'tsp-social-share',
            plugins_url('../assets/js/tsp-social-share.js', __FILE__),
            [],
            '1.1.0',
            true
        );
    ...
    {
        if (!is_singular() || !get_option('tsp_ss_enabled', 1)) return '';

        $networks  = get_option('tsp_ss_networks', ['facebook','twitter','whatsapp','linkedin','telegram']);
        $label     = get_option('tsp_ss_label', 'Share this post:');
    ...
    public function inject_share_buttons($content)
    {
        ...
        if (get_option('tsp_ss_position', 'below_content') !== 'below_content') return $content;
        if (!get_option('tsp_ss_enabled', 1)) return $content;
    ...
    {
        if (!is_singular()) return;
        if (get_option('tsp_ss_position', 'below_content') !== 'floating') return;
        if (!get_option('tsp_ss_enabled', 1)) return;
